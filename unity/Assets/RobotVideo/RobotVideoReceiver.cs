...
public sealed class RobotVideoReceiver : MonoBehaviour
{
    ...
    public Vector2 windowSize = new Vector2(560f, 315f);
    public Vector2 topRightMargin = new Vector2(24f, 24f);
    public Vector3 worldSpaceLocalPosition = new Vector3(0.38f, 0.18f, 1.15f);
    [Min(0.0001f)] public float worldSpaceScale = 0.00075f;
    public int canvasSortOrder = 50;

    private Texture2D currentTexture;
    private AspectRatioFitter aspectRatioFitter;
    private GameObject generatedCanvas;
    private Canvas generatedCanvasComponent;
    private RectTransform generatedWindowRect;
    ...
    private void OnEnable()
    {
        ...
        {
            // The operator view must show the physical camera mounted on TonyPi.
    ...
    private void OnDisable()
    {
        ...
        if (generatedCanvas != null)
        {
            ...
            generatedCanvasComponent = null;
            generatedWindowRect = null;
    ...
    private void EnsureTargetImage()
    {
        ...
        generatedCanvasComponent = generatedCanvas.GetComponent<Canvas>();
        generatedCanvasComponent.renderMode = RenderMode.ScreenSpaceOverlay;
        generatedCanvasComponent.sortingOrder = canvasSortOrder;
        ...
        generatedWindowRect = window.GetComponent<RectTransform>();
        generatedWindowRect.sizeDelta = windowSize;

        Image background = window.GetComponent<Image>();
        background.color = new Color(0.015f, 0.02f, 0.025f, 0.96f);
        background.raycastTarget = false;
        Outline frame = window.GetComponent<Outline>();
        frame.effectColor = new Color(0.15f, 0.85f, 1f, 0.95f);
        ...
        GameObject labelObject = new GameObject(
            ...
            typeof(Text),
            typeof(Outline));
        labelObject.transform.SetParent(window.transform, false);
        RectTransform labelRect = labelObject.GetComponent<RectTransform>();
        labelRect.anchorMin = new Vector2(0f, 1f);
        labelRect.anchorMax = new Vector2(0f, 1f);
        labelRect.pivot = new Vector2(0f, 1f);
        labelRect.anchoredPosition = new Vector2(12f, -10f);
        labelRect.sizeDelta = new Vector2(330f, 30f);
        connectionLabel = labelObject.GetComponent<Text>();
        connectionLabel.font = Resources.GetBuiltinResource<Font>("LegacyRuntime.ttf");
        connectionLabel.fontSize = 18;
        connectionLabel.fontStyle = FontStyle.Bold;
        connectionLabel.alignment = TextAnchor.UpperLeft;
        connectionLabel.raycastTarget = false;
    ...
    {
        if (!worldSpaceForVr || generatedCanvasComponent == null)
            return;

        Camera candidate = FindPresentationCamera();
        if (candidate == null || candidate == presentationCamera)
            return;

        presentationCamera = candidate;
        generatedCanvasComponent.renderMode = RenderMode.WorldSpace;
        generatedCanvasComponent.worldCamera = presentationCamera;
        generatedCanvas.transform.SetParent(presentationCamera.transform, false);
        ...
        RectTransform canvasRect = generatedCanvas.GetComponent<RectTransform>();
        canvasRect.sizeDelta = windowSize;
        generatedWindowRect.anchorMin = new Vector2(0.5f, 0.5f);
        generatedWindowRect.anchorMax = new Vector2(0.5f, 0.5f);
        generatedWindowRect.pivot = new Vector2(0.5f, 0.5f);
        generatedWindowRect.anchoredPosition = Vector2.zero;
    }

    private Camera FindPresentationCamera()
    {
        Camera[] cameras = Camera.allCameras;
        for (int index = 0; index < cameras.Length; index++)
        {
            Camera candidate = cameras[index];
            if (candidate != null && candidate.isActiveAndEnabled && candidate.stereoEnabled)
                return candidate;
        }
    ...
    {
        generatedWindowRect.anchorMin = Vector2.one;
        generatedWindowRect.anchorMax = Vector2.one;
        generatedWindowRect.pivot = Vector2.one;
        generatedWindowRect.anchoredPosition = new Vector2(-topRightMargin.x, -topRightMargin.y);
    }
    ...
    private void UpdateConnectionLabel()
    {
        ...
        connectionLabel.text = IsConnected
            ? "TONYPI CAMERA  LIVE"
            : "TONYPI CAMERA  CONNECTING";
        connectionLabel.color = IsConnected
