...
public class RobotSyncManager : MonoBehaviour, IRobotCommandSink, IRobotStateProvider
{
    ...
    public Vector2 CurrentManualInput { get; private set; }
    ...
    void Update()
    {
        ...
        // Read the same manual input continuously for both local XR locomotion
        // and TonyPi control. Connecting the robot must not disable either path.
        {
            // Move
            if (moveAction != null && moveAction.action != null && moveAction.action.enabled)
            {
            ...
            // PICO Live Preview can expose controller state through the XR input
            // subsystem even when the Input System action reports zero.
            if (move == Vector2.zero)
                move = ReadXrStick(XRNode.LeftHand);
            ...
            if (turnAction != null && turnAction.action != null && turnAction.action.enabled)
            {
                ...
                if (act.expectedControlType == "Vector2")
                {
                    try { rotate = act.ReadValue<Vector2>(); } catch { rotate = Vector2.zero; }
                    gotVector2 = true;
                }
                else
                {
            ...
            if (rotate == Vector2.zero)
                rotate = ReadXrStick(XRNode.RightHand);

            // Desktop gamepad fallback.
            var gamepad = Gamepad.current;
            if (gamepad != null)
            {
                if (move == Vector2.zero) move = gamepad.leftStick.ReadValue();
                if (rotate == Vector2.zero) rotate = gamepad.rightStick.ReadValue();
            }

            // Keyboard and controller share the same command values.
            if (Keyboard.current != null)
            {
                float keyboardForward = (Keyboard.current.wKey.isPressed ? 1f : 0f)
                    - (Keyboard.current.sKey.isPressed ? 1f : 0f);
                float keyboardTurn = (Keyboard.current.dKey.isPressed ? 1f : 0f)
                    - (Keyboard.current.aKey.isPressed ? 1f : 0f);
                if (keyboardForward != 0f) move.y = keyboardForward;
                if (keyboardTurn != 0f) rotate.x = keyboardTurn;
            }
            ...
            CurrentManualInput = new Vector2(manualSteer, manualV);
        ...
        //}
        HandleButtonEdge(rightGripAction, XRNode.RightHand, XrControlFallback.Grip,
            ref _rightGripHeldPrev, rightGripCommend, true);
        HandleRecoveryHold(rightTriggerAction, XRNode.RightHand,
            ref _rightTriggerHoldSince, ref _rightRecoverySent, rightTriggerCommend);
        HandleRecoveryHold(leftTriggerAction, XRNode.LeftHand,
            ref _leftTriggerHoldSince, ref _leftRecoverySent, leftTriggerCommend);
    ...
    private enum XrControlFallback
    {
        Grip,
        Trigger
    }

    private static Vector2 ReadXrStick(XRNode node)
    {
        UnityEngine.XR.InputDevice device = InputDevices.GetDeviceAtXRNode(node);
        return device.isValid && device.TryGetFeatureValue(UnityEngine.XR.CommonUsages.primary2DAxis, out Vector2 value)
            ? value
            : Vector2.zero;
    }
    ...
    private void HandleButtonEdge(
        ...
        XRNode node,
        XrControlFallback fallback,
        ref bool heldPrev,
        string cmd,
        bool triggerLocalAction)
    {
        bool heldNow = ReadHeld(actionRef) || ReadXrHeld(node, fallback);
    ...
    private static bool ReadXrHeld(XRNode node, XrControlFallback fallback)
    {
        UnityEngine.XR.InputDevice device = InputDevices.GetDeviceAtXRNode(node);
        if (!device.isValid) return false;

        InputFeatureUsage<bool> buttonUsage = fallback == XrControlFallback.Grip
        ...
        InputFeatureUsage<float> analogUsage = fallback == XrControlFallback.Grip
            ? UnityEngine.XR.CommonUsages.grip
            : UnityEngine.XR.CommonUsages.trigger;
        return device.TryGetFeatureValue(analogUsage, out float value) && value > 0.5f;
    }
    ...
    private void HandleRecoveryHold(
        ...
        string cmd)
    {
        float value = Mathf.Max(ReadAnalog(actionRef), ReadXrAnalog(node, XrControlFallback.Trigger));
    ...
    private static float ReadXrAnalog(XRNode node, XrControlFallback fallback)
    {
        UnityEngine.XR.InputDevice device = InputDevices.GetDeviceAtXRNode(node);
        if (!device.isValid) return 0f;

        InputFeatureUsage<float> usage = fallback == XrControlFallback.Grip
            ? UnityEngine.XR.CommonUsages.grip
            : UnityEngine.XR.CommonUsages.trigger;
        ...
        InputFeatureUsage<bool> buttonUsage = fallback == XrControlFallback.Grip
            ? UnityEngine.XR.CommonUsages.gripButton
            : UnityEngine.XR.CommonUsages.triggerButton;
        return device.TryGetFeatureValue(buttonUsage, out bool pressed) && pressed ? 1f : 0f;
    }
