...
public class LightControl : MonoBehaviour
{
    ...
    public bool spawnDetectedFireInView = true;
    [Min(1f)] public float detectedFireDistance = 4f;
    ...
    private void OnBallDetected(string targetId)
    {
        ...
        if (key == DefaultKey && spawnDetectedFireInView)
            TryGetVisibleSpawnPose(ref spawnPos, ref spawnRot);
    ...
    {
        OnBallDetected(DefaultKey);
    }

    private void TryGetVisibleSpawnPose(ref Vector3 spawnPos, ref Quaternion spawnRot)
    {
        Camera viewer = Camera.main;
        if (viewer == null)
            viewer = FindFirstObjectByType<Camera>();
        if (viewer == null)
            return;
        ...
        Vector3 candidate = viewer.transform.position + forward * Mathf.Max(1f, detectedFireDistance);
        Vector3 probeOrigin = candidate + Vector3.up * 3f;
        if (Physics.Raycast(probeOrigin, Vector3.down, out RaycastHit hit, 12f,
                Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore))
            candidate.y = hit.point.y + spawnYOffset;
        else if (robotRoot != null)
            candidate.y = robotRoot.position.y + spawnYOffset;
        else
            candidate.y = viewer.transform.position.y - 1.5f + spawnYOffset;

        spawnPos = candidate;
        spawnRot = Quaternion.LookRotation(forward, Vector3.up);
        Debug.Log($"[LightControl] Visible fire spawn at {spawnPos}");
    }
